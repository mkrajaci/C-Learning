...
unsigned funkcija_broj_poklona(struct Poklon*, unsigned, char*);
int main(void) 
{
	...
	if (broj_poklona)
	{
/*		struct Poklon *pokloni = calloc(broj_poklona, sizeof(struct Poklon));	// Alokacija kolicine memorije prema broju poklona
		for (brojac = 0; brojac < broj_poklona; ++brojac)		// punjenje strukture podacima
		{
			printf("Unesite %d. ime: ", ++brojac);
			...
			scanf("%u", &pokloni[brojac].broj_godina);			// Koristim & jer ovo nije niz kao ostale varijable strukture
		}*/
		struct Poklon pokloni[] =							// punjenje strukture podacima
		{
...
unsigned funkcija_broj_poklona(struct Poklon *poklon, unsigned broj_poklona, char *ime_poklona) 
{
